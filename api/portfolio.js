...
export default async function handler(req) {
  if (req.method === 'GET') {
    try {
      ...
      return new Response(JSON.stringify({ message: 'Erreur serveur lors de la lecture des données.' }), {
        status: 500,
        headers: { 'Content-Type': 'application/json' },
      });
    }
  ...
  if (req.method === 'POST') {
    try {
      ...
      return new Response(JSON.stringify({ message: 'Données sauvegardées avec succès.' }), {
        status: 200,
        headers: { 'Content-Type': 'application/json' },
      });
    } catch (error) {
      console.error('Erreur d'écriture dans Vercel KV:', error);
      return new Response(JSON.stringify({ message: 'Erreur serveur lors de la sauvegarde des données.' }), {
        status: 500,
        headers: { 'Content-Type': 'application/json' },
      });
    }
